...
def set_title(slide, title_text, subtitle_text=None):
    ...
    for ph in slide.placeholders:
        ...
        if idx == 0:
            ph.text = title_text
            # RULE-04 단일 팔레트: 제목도 테마 accent(#C72128)로 통일 +
            # RULE-02 제목 자간 -50. (템플릿 기본 dark-red schemeClr 덮어씀)
            for p in ph.text_frame.paragraphs:
                for r in p.runs:
                    r.font.color.rgb = HCG_RED
                    set_char_spacing(r, -50)
...
def build_section_divider(prs, section_num, section_title, sub):
    ...
    cp = ctf.paragraphs[0]
    cp.alignment = PP_ALIGN.CENTER
    _scn = _add_run(cp, section_num, fsize=22, bold=True, color=WHITE)
    set_text_outline(_scn, "FFFFFF", 0.75)   # RULE-02 텍스트 윤곽선
...
def build_body_2col(prs, title, subtitle, header_l, header_r, left_items, right_items):
    ...
    add_item(slide, header_l, COL_L_X, HDR_Y, width=ITEM_W, height=0.333,
             fsize=11, bold=True, align=PP_ALIGN.CENTER,
             fill_rgb=HCG_RED, color=WHITE, no_border=True,
             shadow=True, outline=_HDR_OUTLINE)
...
_HDR_OUTLINE = {"color": "FFFFFF", "width_pt": 0.75}   # RULE-02 accent 헤더밴드 윤곽선
_TAG_OUTLINE = {"color": "FFFFFF", "width_pt": 0.5}

def add_header_bar(slide, text, x=0.691, y=1.595, w=9.451, h=0.333, fill=None):
    """콘텐츠 영역 헤더 바 (accent fill + 흰 bold + 윤곽선·그림자)."""
    return add_item(slide, text, x, y, width=w, height=h, fsize=11, bold=True,
                    align=PP_ALIGN.LEFT, fill_rgb=fill or MED_GRAY, color=WHITE,
                    no_border=True, shadow=True, outline=_HDR_OUTLINE)

def add_label_tag(slide, text, x, y, w=1.02, h=0.5, fill=None):
    """미니 카테고리 라벨 태그 (윤곽선·그림자)."""
    return add_item(slide, text, x, y, width=w, height=h, fsize=9, bold=True,
                    align=PP_ALIGN.CENTER, fill_rgb=fill or GRAY_DK, color=WHITE,
                    no_border=True, shadow=True, outline=_TAG_OUTLINE)

def add_connector_badge(slide, text, x=4.752, y=1.994, w=1.25, h=0.39, fill=None):
    """중앙 커넥터/키워드 배지 (윤곽선·그림자)."""
    return add_item(slide, text, x, y, width=w, height=h, fsize=9, bold=True,
                    align=PP_ALIGN.CENTER, fill_rgb=fill or MED_GRAY, color=WHITE,
                    no_border=True, shadow=True, outline=_TAG_OUTLINE)
...
def add_vs_badge(slide, x=5.22, y=1.61, w=0.39, h=0.39, fill=None, text="VS"):
    ...
    r = _add_run(p, text, fsize=10, bold=True, color=WHITE)
    set_text_outline(r, "FFFFFF", 0.5)
    add_shadow(sh)
    return sh
...
def build_overview_3col(prs, title, subtitle, cols, bar_label=None, example=False):
    ...
    for i, col in enumerate(cols[:3]):
        x = xs[i]
        add_item(slide, "", x, 2.88, width=w, height=4.17, fill_rgb=GRAY_LT, no_border=True, shadow=True)
        ...
        add_item(slide, col.get("ai", ""), x + 0.10, 3.86, width=w - 0.20, height=1.45, fsize=9, bold=True,
                 ...
                 shadow=True, outline=_TAG_OUTLINE)
...
def build_diff_matrix(prs, title, subtitle, rows, bottom_quote=None, example=False):
    ...
    for i, r in enumerate(rows[:4]):
        ...
        add_item(slide, ins, 4.21, y, width=2.68, height=0.97, fsize=9, bold=True,
                 align=PP_ALIGN.CENTER, fill_rgb=GRAY_LT, color=HCG_RED, shadow=True)
...
def build_pain_point_categorized(prs, title, subtitle, left_hdr, right_hdr, rows,
    ...
    if summary_left:
        add_item(slide, summary_left, 0.691, 5.62, width=4.528, height=0.92, fsize=10, bold=True,
                 align=PP_ALIGN.CENTER, fill_rgb=GRAY_LT, color=DARK_GRAY, no_border=True, shadow=True)
    if summary_right:
...
def build_approach_vs(prs, title, subtitle, left_title, left_items, right_title, right_items,
                      bottom_quote=None, bullet="• "):
    """Legacy vs HCG 'VS' 대비 블록 + 하단 takeaway.
    bullet : 항목 머리표 (기본 '• '). ''(빈문자) 전달 시 글머리 글자 없이 렌더
             → RULE-01(불릿 나열 0장) 준수용. lotte/kia 기존 호출자는 기본값 유지."""
    slide = prs.slides.add_slide(prs.slide_layouts[2])
    set_title(slide, title, subtitle)
    add_item(slide, "", 0.69, 2.20, width=4.52, height=4.10, fill_rgb=GRAY_LT, no_border=True, shadow=True)
    add_textbox(slide, left_title, 0.80, 2.30, 4.30, 0.40, fsize=12, bold=True, color=DARK_GRAY)
    yy = 2.92
    for it in left_items:
        add_textbox(slide, (bullet + it) if bullet else it, 0.92, yy, 4.08, 0.70, fsize=10, color=DARK_GRAY); yy += 0.74
    add_item(slide, "", 5.65, 2.20, width=4.45, height=4.10, fill_rgb=None, no_border=False, shadow=True)
    add_textbox(slide, right_title, 5.78, 2.30, 4.20, 0.40, fsize=12, bold=True, color=HCG_RED)
    yy = 2.92
    for it in right_items:
        add_textbox(slide, (bullet + it) if bullet else it, 5.88, yy, 4.05, 0.70, fsize=10, color=DARK_GRAY); yy += 0.74
            ...
            for p in cell.text_frame.paragraphs:
                ...
                for run in p.runs:
                    ...
                    if header and r == 0:
                        ...
                        set_text_outline(run, "FFFFFF", 0.5)   # RULE-02 헤더 윤곽선
            cell.fill.solid()
    ...
    for i, ph in enumerate(phases):
        ...
        for st in ph.get("steps", []):
            add_item(slide, st, x, sy, width=pw, height=0.66, fsize=9, align=PP_ALIGN.CENTER,
                     fill_rgb=GRAY_LT, color=DARK_GRAY, no_border=True, shadow=True)
            sy += 0.74
        if i < n - 1:
...
def create_toc_slide(prs, sections, current=None, layout_idx=1,
    ...
    for i, sec in enumerate(sections):
        ...
        rr = _add_run(rp, ROMAN[i] if i < len(ROMAN) else str(i + 1),
                      fsize=18, bold=True, color=WHITE)
        set_text_outline(rr, "FFFFFF", 0.75)   # RULE-02 accent 밴드 텍스트 윤곽선
        ...
        if scope:
            ...
            for t, x in scope:
                add_item(slide, t, x, 2.58, width=1.976, height=0.62,
                         ...
                         color=WHITE, no_border=True, shadow=True, outline=_HDR_OUTLINE)
